...
"
    ...
    do
        ...
        if [ -n "${TARGET_CC}" ]; then
            ...
            CXX_FOR_TARGET=${TARGET_CXX} CC_FOR_TARGET=${TARGET_CC} CGO_ENABLED=1 GOPRIVATE=github-chf01.synamedia.com/COS GOOS=${TARGET_OS} GOARCH=${TARGET_ARCH} ${GO_BINARY} build
        else
            CGO_ENABLED=1 GOPRIVATE=github-chf01.synamedia.com/COS GOOS=${TARGET_OS} GOARCH=${TARGET_ARCH} ${GO_BINARY} build
        fi
        let -i result=$?
        if [ ${result} -ne 0 ]; then
            break
...
{
    let -i ret=0
    CGO_ENABLED=1 GOPRIVATE="github-chf01.synamedia.com/COS" go fmt ./...
    return ${ret}
}
...
function runUnitTests()
{
    local GO_BINARY="${1}"
    printf "\nRunning unit tests...\n"
    CGO_ENABLED=1 GOPRIVATE="github-chf01.synamedia.com/COS" ${GO_BINARY} test '-coverprofile=coverage.txt' -covermode count -test.short -failfast -v ./...
    let -i ret=$?
    return ${ret}
}
...
function runIntegrationTests()
{
    ...
    do
        case "${1}" in
            ...
            "--debug")
                ...
                GO_BINARY="dlv test --build-flags='github-chf01.synamedia.com/COS/goringtrace'  --"
                ;;
            *)
    ...
    CGO_ENABLED=1 GOPRIVATE="github-chf01.synamedia.com/COS" ${GO_BINARY} -test.failfast ${INTEGRATION_TEST_OPTIONS} -test.v -test.timeout ${RUN_TIMEOUT}s . ${RUN_TEST_SUITE} ${RUN_TEST_SUITE_OPTS}
    let -i ret=$?
    return ${ret}
}
